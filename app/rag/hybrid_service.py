...
class HybridRAGService:
    ...
    def _initialize_model(self):
        ...
        try:
            model_name = self.config.get("rag", {}).get("model", "sentence-transformers/all-MiniLM-L6-v2")
    ...
    async def generate_reply(self, conversation_id: int, user_text: str) -> RAGResult:
        ...
        response_text = await self.process_query(user_text)
    ...
    async def suggest_topics(self, conversation_id: int, user_text: str, answer_text: str) -> List[str]:
    ...
    async def generate_ticket_summary(self, messages: List[Any], ticket_id: int = None) -> str:
        ...
        try:
            ...
            for msg in messages:
                ...
                content = getattr(msg, 'text', '') or getattr(msg, 'content', '') or str(msg)
...
Краткое резюме (максимум 2-3 предложения):"""
            ...
            if hasattr(self, 'rag_service') and self.rag_service and hasattr(self.rag_service, 'llm_client'):
            ...
            if messages:
                first_msg = getattr(messages[0], 'text', '') or getattr(messages[0], 'content', '') or str(messages[0])
        ...
        try:
            async with get_knowledge_session()() as session:
                ...
                if best_match and best_score > 0:
                    content = best_match.content[:300] + "..." if len(best_match.content) > 300 else best_match.content

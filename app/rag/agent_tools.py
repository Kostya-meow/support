...
async def search_knowledge_base(query: str, category: str = None) -> str:
    ...
    try:
        ...
        async with KnowledgeSessionLocal() as session:
            ...
            response = "\n\n".join(response_parts)
            print(f"[AGENT] Возвращаю результат поиска (длина: {len(response)} символов)")
            return response
...
def improve_search_query(original_query: str, context: str = None) -> str:
    ...
    try:
        ...
        # Формируем промпт для улучшения запроса
        improvement_prompt = f"""Ты - эксперт по поисковым запросам. Твоя задача - улучшить поисковый запрос для более точного поиска в базе знаний технической поддержки.

Исходный запрос: "{original_query}"
Контекст: {context if context else "Техническая поддержка"}
...
@tool 
def classify_request(dialogue_history: str = None, text: str = None, categories: List[str] = None) -> str:
    ...
    if dialogue_history:
        ...
        print(f"[AGENT ACTION] Классификация диалога через LLM (длина: {len(dialogue_history)} символов)")
    elif text:
        analysis_text = text
    ...
    try:
        ...
        default_categories = ["Инфраструктура", "Разработка", "Сеть", "Безопасность", "Пользовательское ПО", "Аппаратные проблемы", "Общий"]
        available_categories = config.get("agent", {}).get("categories", categories or default_categories)
        
        # Формируем промпт для классификации
        categories_text = ", ".join(available_categories)
        
        classification_prompt = f"""Проанализируй диалог с пользователем и определи категории проблем.
...
Категории:"""
        ...
        if not valid_categories:
            ...
            category_keywords = {
                "Инфраструктура": ["сервер", "сеть", "железо", "оборудование", "инфраструктура"],
                "Разработка": ["код", "программа", "баг", "ошибка", "приложение", "разработка"],
                "Сеть": ["интернет", "сеть", "роутер", "wifi", "подключение"],
                "Безопасность": ["пароль", "доступ", "права", "безопасность", "блокировка"],
                "Пользовательское ПО": ["программа", "софт", "приложение", "установка"],
                "Аппаратные проблемы": ["компьютер", "принтер", "мышь", "клавиатура", "монитор"],
                "Общий": []
            }
            ...
            if scores:
                ...
                valid_categories = [cat for cat, score in sorted_categories[:2]]  # Максимум 2 категории
            else:
...
def call_operator() -> str:
    ...
    print("[AGENT ACTION] ВЫЗОВ ОПЕРАТОРА! Передача сложного запроса живому специалисту")
...
def get_system_status() -> str:
    ...
    try:
        ...
        return f"Система работает нормально. Время: {current_time}. База знаний доступна."

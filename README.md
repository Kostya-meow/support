...
- **LLM модели** (OpenAI-совместимый API):
  - Gemma 2 7B (основная модель для диалогов)
  - Gemma 2 9B (альтернативная модель)
...
- **VK Bot** (vk_api) - дополнительный канал
...
LLM_MODEL=gemma-2-7b  # или gemma-2-9b, gpt-3.5-turbo
```

Доступные модели:
- **gemma-3-27b** - основная рабочая модель 
- **gemma-3-9b** - более мощная альтернатива
...
### Добавление новых MCP функций
1. Создать функцию в `app/rag/agent_tools.py`
2. Добавить декоратор `@tool`
3. Документировать параметры и возвращаемое значение
4. Добавить в список инструментов агента в `app/rag/agent.py`
5. Опционально: добавить действие в `_send_action_to_telegram()`
...
- [ ] Поддержка вложений в веб-чате
- [ ] Email нотификации
- [ ] Интеграция с внешними системами (Jira, ServiceNow)
...
Прототип для внутреннего использования. Все права защищены.
